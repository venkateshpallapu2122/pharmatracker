       ...
       <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full sm:w-auto"><Filter className="mr-2 h-4 w-4" /> Filter Status</Button>
            </DropdownMenuTrigger>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="font-headline cursor-pointer group hidden md:table-cell" onClick={() => handleSort('priority')}>Priority {renderSortIcon('priority')}</TableHead>
                ...
                <TableHead className="font-headline cursor-pointer group hidden sm:table-cell" onClick={() => handleSort('status')}>Status {renderSortIcon('status')}</TableHead>
              ...
              {sortedTasks.length > 0 ? (
                sortedTasks.map((task) => (
                  <TableRow key={task.id} className="hover:bg-muted/30 transition-colors">
                    <TableCell className="font-medium max-w-[150px] sm:max-w-xs truncate" title={task.title}>{task.title}</TableCell>
